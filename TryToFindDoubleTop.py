...
x = datetime.datetime(2022,7,22,21,0,2)
...
while True:
    # print("-----------------")
    a=conn.cursor()
    sql = 'select happentime,lastprice from pta2209_20220725 where happentime<=%s and hour(happentime)>=21  order by happentime desc limit 2 ;'
    a.execute(sql,x)
    data=a.fetchall()
    conn.commit()
    # print(x)
    ...
    if len(t)>240:
        slope, intercept, r_value, p_value, std_err = stats.linregress(t[-240:],s[-240:])
